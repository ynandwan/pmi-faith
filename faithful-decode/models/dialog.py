...
class DialogueTransformer(BaseTransformer):
    ...
    def training_step(self, batch, batch_nb):
        ...
        if torch.isnan(lm_loss.mean()):
            print("********** FOUND Nan loss ************", lm_loss)

        lm_loss = lm_loss[~lm_loss.isnan()]
        ...
        if loss.isnan().item():
            print("HOW IS IT POSSIBLE AFTER FILTERING???********** FOUND Nan loss ************", loss)
    ...
    def validation_epoch_end(self, outputs: List[torch.Tensor]):
        ...
        save_path = self.output_dir.joinpath("last_model")
